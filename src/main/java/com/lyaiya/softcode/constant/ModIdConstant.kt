package com.lyaiya.softcode.constant

object ModIdConstant {
    const val FORGELIN_CONTINUOUS = "forgelin_continuous"
    const val MIXIN_BOOTER = "mixinbooter"

    const val IN_WORLD_CRAFTING = "inworldcrafting"
    const val EX_NIHILO_CREATIO = "exnihilocreatio"
    const val THAUMCRAFT = "thaumcraft"
    const val THAUMIC_JEI = "thaumicjei"
    const val IMMERSIVE_ENGINEERING = "immersiveengineering"
    const val BLOCK_DROPS = "blockdrops"
    const val IC2 = "ic2"
    const val CONTROLLING = "controlling"
    const val LIB_VULPES = "libvulpes"
    const val ADVANCED_ROCKETRY = "advancedrocketry"
    const val SERENES_SEASONS = "sereneseasons"
    const val JUST_ENOUGH_RESOURCES = "jeresources"
}